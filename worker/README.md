...
`POST /api/rankings` records or updates submitted categories. `DELETE /api/rankings` removes the current network's submitted ranking, or `DELETE /api/rankings?category=Breakfast` removes only one category. `GET /api/rankings` returns community standings grouped by category, with vote counts and average rank.
